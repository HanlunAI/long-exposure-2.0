...
async function main() {
    ...
    async function processVideo() {
        ...
        var rotated = new Uint8ClampedArray(frame.data);
        for (var i = 0; i <  frame.cols; i++ ) {
            for ( var j = 0 ; j < frame.rows; j++) {
                for (var k = 0; k<4; k++){
                    rotated[(i*frame.cols + j)*4+k] = frame.data[((frame.rows-j-1) * frame.cols + i)*4+k];
                }
                
            }
        }
        console.log(rotated.length,frame.data.length)

        var imgData = new ImageData(rotated, frame.rows, frame.cols);
